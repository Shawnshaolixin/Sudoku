...
> 版本:**v2.1** | 状态:Draft(可执行) | 基线:现有 Unity 数独工程(已按 §1.4 逐项核对真实工程文件)
...
> **v2.1 变更**:D-10 落实(`.gitignore` 已移除本文与 07 的忽略规则,本文正式入库);新增 **D-11 UI 层方案 + §3.5 `UIKit` 薄层**(明确不引入 GameFramework 类全家桶);新增 **D-12 引擎版本 + §4.9 排除法与迁移 checklist**;§1.4 补 **Unity IAP 未安装**、AdMob 8.7.0;P-1/P0-b 工期与验收随之调整。
...
| **D-12** | 引擎版本 | **⚠️ 推荐:换国际版,并优先当前 LTS 家族最新补丁;排除法见 §4.9;不满足则回退 2022.3 国际版最终补丁** | 出海项目应使用国际版(插件/文档/政策一致);2022 LTS 已过官方支持窗口,而 Play 的 targetSdk 要求每年上调,越晚迁移越贵。工程现在只有 4 个 asmdef / 2 个场景,**这是迁移成本最低的时刻** |
...
- **引擎**:Unity 2022 LTS + UGUI + TMP(对齐 [03_技术架构.md](./03_技术架构.md))
...
| Unity 版本 | "Unity 2022 LTS" | **2022.3.50f1c1**(Unity 中国版 c1 分支) | `ProjectSettings/ProjectVersion.txt` |
...
- **换国际版**:出海项目不建议留在中国版 `c1` 分支 —— 插件生态、官方文档、Play 工具链与政策说明都以国际版为基准,排障时少一层变量
- **选版原则**:选 **"官方当前标注 LTS 的家族" 的最新补丁**,不选 Tech Stream / 非 LTS 中间版本(HybridCLR、Addressables、AdMob/Firebase 对新版本的适配总是滞后)
- **回退方案**:若 LTS 家族任一硬约束不满足 → 回退 **2022.3 国际版最终补丁**(与现工程同小版本,迁移几乎零 API 改动),但要接受"已过官方支持窗口 + targetSdk 年度上调会越来越吃力"
- **时机**:**必须在 P-1 做**,现在工程只有 4 个 asmdef / 2 个场景 / 无 Addressables 无 HybridCLR —— 这是迁移最便宜的时刻;等 P0 之后再迁,要连带重验热更管线

#### 排除法(30 分钟可完成,顺序不可乱)

| 步 | 查什么 | 硬约束 |
|---|---|---|
| 1 | HybridCLR [支持版本页](https://www.hybridclr.cn/en/docs/8.5.0/basic/supportedplatformanduniyversion) | 目标版本必须在支持列表内(否则 §4.4 整章作废) |
| 2 | Play [目标 API 级别要求](https://developer.android.com/google/play/requirements/target-sdk) | 目标 Unity 补丁必须能出**当前强制 targetSdk** 的 AAB(政策每年上调,以该页当前值为准) |
| 3 | Unity 下载归档 | 在 ① ∩ ② 里取**当前 LTS 家族的最新补丁** |
| 4 | 插件兼容 | Addressables(2.x 与 1.x API 有差异)、Firebase Unity SDK 13.15、GoogleMobileAds 8.7.0、Unity IAP 各自 release notes 支持该 Unity 版本 |
| 5 | 任一不满足 | 回退 2022.3 国际版最终补丁,并在文档记录原因与复查时间 |

> 说明:本文不写死具体补丁号 —— 补丁与政策日期会变,写死必然过期。**决策规则写死,版本号由上表现场查**;定版后把结论(版本号 + 查验日期 + 五项结论)回填到本节。

#### 迁移 checklist(P-1 执行)

1. **先提交/打 tag**,再复制一份工程做迁移分支(不要原地升)
2. 删除 `Library/`、`obj/`,用目标版本 Unity Hub 打开,允许 API Updater 跑完
3. 处理升级弹窗:TMP 资源升级、Input System(若提示)、色彩空间/图集设置保持不变
4. **EDM4U 重新 Resolve**(Firebase / AdMob 的 Android 依赖),确认 Gradle / JDK / NDK 用 Hub 装的对应模块
5. 保留 `scriptingDefineSymbols`(`SUDOKU_ADMOB;SUDOKU_FIREBASE`,接 IAP 后加 `SUDOKU_IAP`)
6. 重配 Player Settings:**IL2CPP + ARM64 + AAB + targetSdk + managedStrippingLevel**(§1.4 的五项债务一次性清掉)
7. `BuildScript.cs` 改为产出 **AAB**(`EditorUserBuildSettings.buildAppBundle = true`,输出 `.aab`)
8. 验收:EditMode 测试全绿 + 真机跑通"启动→单局→激励广告→返回"链路 + §11 五项基线重测一遍
9. 迁移后回填本节结论,并同步 03 文档的引擎/minSdk 表述
...
| Phase | 内容 | 验收标准 | 工期 |
|---|---|---|---|
| **P-1 前置(硬门槛)** | **引擎定版并迁移(D-12 / §4.9 排除法 + 迁移 checklist)**;工程债务清理:**ARM64 + IL2CPP + AAB + targetSdk + managedStrippingLevel** 一次配齐;`BuildScript` 改出 AAB;开局生成移出主线程;Canvas 静/动分层;Profiler 基线与工作流 | 目标 Unity 版本下 IL2CPP+ARM64 的 AAB 真机可跑;EditMode 测试全绿;开局不阻塞首帧;§11 五项基线有数;§4.9 结论已回填 | 1.5~2 周 |
...
- **P-1 → v1.0 上架合计**:(1.5~2) + 1.5 + (2.5~3) + 1.5 + 1.5 + (1.5~2) = **10~11.5 周(净开发)**
- 加 25% 缓冲(商店往返、材料、合规、返工)→ **13~14 周,合理区间 12~15 周(约 3~3.5 个月)**
...
1. **D-12 引擎版本**:按 §4.9 排除法定版(**换国际版当前 LTS 家族最新补丁**为推荐路线;回退项 = 2022.3 国际版最终补丁)—— 这是 P-1 的第一个动作,不定就开不了工
2. **D-4 玩法数量**:v1.0 做 2 个玩法(推荐)还是 3 个?
3. **D-8 HybridCLR 版本**:社区版起步(推荐)还是直接商业版?
4. **D-9 广告聚合**:AdMob 单栈(推荐)还是直接上聚合?
5. **Unity IAP**:P-1 一并接通"去广告"(推荐,现状是 `#if SUDOKU_IAP` 未启用)还是推到 P4?
6. **SSV**:P4 接受"本地校验 + 频控"过渡(推荐)还是直接搭接收端?
7. **R3**:是否引入(建议 P2 之后再评估;P0 只用 UniTask + DI)

已拍板(见 §0):D-1 下发通道、D-2 v1.0 纯 AOT、D-3 Trait 分层、D-5 单货币、D-6 配置四分管、D-7 存档、**D-10 本文入库(`.gitignore` 已改)**、**D-11 不引入全家桶框架,自研 `UIKit`**。
...
| 参数冲突 | ⚠️ 待定 | 03 写 minSdk 24、工程是 23 → **随 D-12 引擎定版一起统一**(定版后同时改工程与 03,不许两处不一致) |
